...
- **A declared error and a fault reply both carry the plain-text header `is_error` = `true`; a success reply carries none.** The payload is unchanged -- this rides the same headers table `header_out` writes into, unencoded because it names no `header_out` value. `header_out("is_error")` is refused at the declaration on a service that asks for `amqp_rpc`, so a bound value never collides with it. A crate that places only `{service}_amqp_rpc_dispatcher!()` builds no `ReplyHandle` and does its own framing over `Reply` by hand, so it is untouched by this.
- `legacy_reply`, `framed_fault` and `outgoing_headers` -- the wire framing a reply and a fault are built through, read back by nothing a generated service sees.
...
A `header_in` binding is one more argument after `req`, a `None` value still sent as JSON `null` rather than left out. A `header_out`/`error_header_out` tuple is rejoined on the way back: an absent optional element reads `null`, a missing or undecodable required one answers the pair's `Fault` member. `{Service}Handlers` takes the same `header_in` arguments, and `attach{Service}WsDispatcher` splits a header-tuple answer the same way it reads one -- a `null` element not written.
